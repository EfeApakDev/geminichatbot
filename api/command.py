...
def get_my_info(id):
    return f"Telegram ID'niz: `{id}`"

def get_group_info(type, chat_id):
    if type == "supergroup":
        return f"Bu grubun ID'si: `{chat_id}`"
    return "Lütfen bu komutu bir grupta kullanın"
...
def speed_test(id):
    """ Bu komut işe yaramaz gibi görünüyor, ancak yaptığım her robota dahil edilmelidir. """
    send_message(id, "Başlatıyorum")
    sleep(5)
    return "Test tamamlandı, 5G hızınız: \n**114514B/s**"
